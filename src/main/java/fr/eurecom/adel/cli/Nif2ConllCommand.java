...
import fr.eurecom.adel.tools.Nif2Conll;
 ...
 * @author Julien Plu
 */
public class Nif2ConllCommand<T extends Configuration> extends AbstractToolsCommand<T> {
  Nif2ConllCommand() {
    super("nif2conll", "Convert NIF annotations into CoNLL annotations");
  }
  ...
  protected final void run(final Namespace newNamespace) throws Exception {
    ...
    final Nif2Conll nif2Conll = new Nif2Conll();
    
    nif2Conll.run(Paths.get(newNamespace.getString("ifile")),
        Paths.get(newNamespace.getString("ofile")), newNamespace.getString("lang"), conf);
