 ...
 * @author Julien Plu
 */
public class NerdCommand<T extends PipelineConfiguration> extends ConfiguredCommand<T> {
  public NerdCommand() {
    super("nerd", "Extract, type and link entities");
  }
  ...
  public final void configure(final Subparser subparser) {
    ...
    subparser.addArgument("-of", "--output-format")
        ...
        .required(false)
        .setDefault("tac")
        .choices("nif", "brat", "tac", "naf", "neel2014")
  ...
  protected final void run(final Bootstrap<T> newBootstrap, final Namespace
    ...
    final Query query = new NerdQuery();
    ...
    final String result = pipeline.nerd(query, newNamespace.getString("lang"), "http://localhost");
